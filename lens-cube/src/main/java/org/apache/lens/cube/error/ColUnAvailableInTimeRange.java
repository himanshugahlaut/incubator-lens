...
public class ColUnAvailableInTimeRange {
  ...
  public String getAvailability() {
    ...
    if (availableFromInMillisSinceEpoch != null && availableTillInMillisSinceEpoch != null) {
      ...
      return "after " + formatTime(DATE_FORMAT, availableFromInMillisSinceEpoch) + " and before "
          + formatTime(DATE_FORMAT, availableTillInMillisSinceEpoch);

    } else if (availableFromInMillisSinceEpoch != null) {

      return "after " + formatTime(DATE_FORMAT, availableFromInMillisSinceEpoch);
    } else {

      return "before " + formatTime(DATE_FORMAT, availableTillInMillisSinceEpoch);
    }
  }

  private String formatTime(final ThreadLocal<DateFormat> dateFormat, final Long time) {
